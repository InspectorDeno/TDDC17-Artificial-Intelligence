...
public class QLearningController extends Controller {
	...
	static final double GAMMA_DISCOUNT_FACTOR = 0.55; /* Must be < 1, small values make it very greedy */
...
//			double previous_reward = StateAndReward.getRewardAngle(previous_angle, previous_vx, previous_vy);
			...
			if (previous_state != null) {
				...
				double rate = alpha(Ntable.get(prev_stateaction));
				double oldQ = Qtable.get(prev_stateaction);
								
				double newQ = oldQ + rate * (previous_reward + 
						(GAMMA_DISCOUNT_FACTOR*getMaxActionQValue(new_state)) - oldQ);
